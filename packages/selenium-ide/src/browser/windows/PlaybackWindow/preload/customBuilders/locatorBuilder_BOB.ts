...
locatorBuilder_BOB.updateAppSpecificOrder = function () {
    ...
    LocatorBuilders.getDisplayName = function(e: any, ignoreInnerText: any) {
        ...
        if (!displayName) {
            ...
            displayName = origDisplayNameFn(e, ignoreInnerText);
        }
        return displayName;
    };
};

function getCustomDisplayNameFn(e: Node, ignoreInnerText: boolean) {
    if ((e as HTMLElement).innerText && (e as HTMLElement).innerText.trim().length > 1 && ignoreInnerText != true)
        return undefined;
    //ignore if element is inside table
    if ((e as HTMLElement).closest('div[class*=ReactVirtualized__Grid],table[id*=-body],table[class*=slds-table_fixed-layout]'))
        return undefined;
    //ignore if element is in dropdown menu/popover edits
    if ((e as HTMLElement).closest('div[class*=slds-dropdown_menu],div[class*=slds-popover]'))
        return undefined;
    var currentNode: any = e;
...
function isElementEligibleForRecordingCustom(e: { getAttribute: (arg0: string) => any; }) {
...
function isElementFoundByLocatorNotMatchedEligibleForRecordingCustom(origEl: { closest: (arg0: string) => any; }, newlyFoundEl: any) {
...
function buildTableExpectedData(e: any, columnAttr: string, parentDivCell: any) {
    var elementType = '', innerText = '';
    var tableExpData:any = {
        elementType : '',
        expectedValue : '',
        elementIndex : 0
    };
    ...
    if (elClassAttr.includes('slds-button') || parButtonEl || childButtonEl) {
        ...
        if (elIdAttr && elIdAttr.includes('-content'))
            ...
            if (!finalEl && e.tagName.toLowerCase() == 'div' && e.getAttribute('role') == 'button' && e.getAttribute(dataColumnAttr) == columnAttr) {
                tableExpData = {
                    elementType : 'BUTTON',
                    expectedValue : e.textContent.trim(),
                    elementIndex : e.parentNode.children.length == 1 ? '1' : '2'
                }
                return tableExpData;
            }
            if (!finalEl) {
    ...
    tableExpData = {
        ...
        expectedValue : innerText
    }
    return tableExpData;
}

function handleVirtualizedTableRecording(e: any) {
    ...
    return;//Gajanan: Added to resolve build issue
}
...
function table(e: any) {
    ...
    var tableRowData = buildTableRowCriteriaData(e);
    if (tableRowData) {
        if (!recordedType) {
            recordedType = 'table';
            additionalData.rowType = 'data';
            additionalData.elementType = tableRowData.elementType;
            additionalData.columnName = tableRowData.columnName;
            additionalData.columnType = tableRowData.columnType;
            additionalData.elementIndex = tableRowData.elementIndex;
        }
